...
if ($search !== '') {
    $where[] = '(reference_no LIKE ? OR method LIKE ? OR notes LIKE ? OR purchase_ref LIKE ?)';
    $like    = '%' . $search . '%';
...
try {
    ...
    if ($row) {
        ...
        $summary['avg_payment']  = (float) $row['avg_payment'];
    }
} catch (Throwable $e) {}
...
try {
    $stmt = $pdo->prepare("
        ...
        FROM payments
        WHERE buyer_id = ? AND MONTH(paid_at) = MONTH(CURRENT_DATE()) AND YEAR(paid_at) = YEAR(CURRENT_DATE())
    ");
    $stmt->execute([$userId]);
...
try {
    ...
    $countParams = [$userId];
    if ($search !== '') {
        $countWhere[] = '(reference_no LIKE ? OR method LIKE ? OR notes LIKE ? OR purchase_ref LIKE ?)';
    ...
    $stmt = $pdo->prepare("SELECT LOWER(method) AS m, COUNT(*) AS c, COALESCE(SUM(amount),0) AS total FROM payments WHERE {$countWhereSql} GROUP BY LOWER(method)");
    $stmt->execute($countParams);
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $methodStats[$row['m'] ?: 'cash'] = [
            'count' => (int) $row['c'],
...
/* --------------------------------------------------------------
   Unpaid purchases (for linking a payment to a purchase)
-------------------------------------------------------------- */
$openPurchases = [];
try {
    $stmt = $pdo->prepare("
        SELECT id, reference_no, seller_name, balance, total_amount
        FROM purchases
        WHERE buyer_id = ? AND balance > 0
        ORDER BY created_at DESC
        LIMIT 100
    ");
    $stmt->execute([$userId]);
    $openPurchases = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Throwable $e) {}
...
<head>
    ...
    <title>Payments | SmartPalay</title>
    ...
    <style>
        /* ============================================================
           SmartPalay — Payments (Premium Edition)
           ============================================================ */
        :root {
            --gold:        #B0641E;
            --gold-2:      #C97628;
...
<aside class="sp-sidebar" id="sidebar">
    ...
    <nav class="sp-nav">
        ...
        <a href="<?= BASE_URL ?>buyer/payments.php" class="sp-nav-link is-active">
    ...
    <main class="sp-content">
        ...
        <section class="sp-intro">
            <div class="sp-intro-text">
                ...
                <h2>Every peso, <em>accounted for</em>.</h2>
                <p>
                    Log payments to your sellers, track settlements, and keep your
                    outstanding balances up to date — all in one clean ledger.
                </p>
                <div class="sp-intro-actions">
                    <button type="button" class="sp-intro-btn is-primary" data-modal="newPayment">
            ...
            <div class="sp-intro-mini">
                <div class="sp-intro-mini-card">
                    <div class="sp-intro-mini-label">Payments</div>
                    <div class="sp-intro-mini-value"><?= number_format($summary['count']) ?></div>
                </div>
                <div class="sp-intro-mini-card">
                    <div class="sp-intro-mini-label">This Month</div>
                    <div class="sp-intro-mini-value"><?= peso($summary['this_month']) ?></div>
                </div>
                <div class="sp-intro-mini-card">
                    <div class="sp-intro-mini-label">Total Paid</div>
                    <div class="sp-intro-mini-value"><?= peso($summary['total_amount']) ?></div>
                </div>
            </div>
        </section>
        ...
        <form class="sp-toolbar" method="get" action="">
            ...
            <div class="sp-method-tabs">
                <?php
                    $tabs = [
                        'all'  => ['All',  array_sum(array_column($methodStats, 'count')), 'bi-grid'],
                    ];
                    $commonMethods = [
                    ...
                    foreach ($commonMethods as $k => $meta) {
                        $tabs[$k] = [$meta[0], $methodStats[$k]['count'] ?? 0, $meta[1]];
                    }
                    foreach ($methodStats as $k => $data) {
                        if (!isset($tabs[$k])) {
                            $tabs[$k] = [ucfirst($k), $data['count'], 'bi-three-dots'];
                        }
                    }
            ...
            <div class="sp-filter-row">
                <div class="sp-field">
                    ...
                    <input type="search" name="q" value="<?= sanitize($search) ?>" placeholder="Search by reference, notes, purchase ref…">
                </div>
                ...
                <a href="<?= BASE_URL ?>buyer/payments.php" class="sp-btn-ghost">
        ...
        <section class="sp-summary-strip">
            ...
            <div class="sp-summary-card">
                ...
                <div class="sp-summary-body">
                    <div class="sp-summary-label">Payments Made</div>
                    ...
                    <table class="sp-table">
                        ...
                        <tbody>
                            <?php foreach ($payments as $p): ?>
                                <?php
                                    [$methodLabel, $methodIco, $methodTone] = methodMeta($p['method'] ?? 'cash');
                                ?>
                                <tr data-payment='<?= htmlspecialchars(json_encode([
                                    ...
                                    <td>
                                        <span class="sp-method-chip <?= $methodTone ?>">
                                            <i class="bi <?= $methodIco ?>"></i> <?= sanitize($methodLabel) ?>
                                        </span>
                                    </td>
            ...
            <?php else: ?>
                <div class="sp-empty">
                    ...
                    <p>
                        ...
                        <?php else: ?>
                            Payments you make to sellers will appear here. Record your first payment to get started.
                        <?php endif; ?>
                    </p>
                    <div class="sp-empty-actions">
                        <?php if ($search !== '' || $methodFilter !== 'all' || $dateFrom || $dateTo): ?>
                            <a href="<?= BASE_URL ?>buyer/payments.php" class="sp-btn-cancel">
                                <i class="bi bi-arrow-counterclockwise"></i> Clear filters
                            </a>
                        <?php endif; ?>
                        <button type="button" class="sp-btn-primary" data-modal="newPayment">
                            <i class="bi bi-plus-lg"></i> Record Payment
                        </button>
                    </div>
                </div>
...
<div class="sp-modal-backdrop" id="modalNewPayment">
    <div class="sp-modal" role="dialog" aria-modal="true" aria-labelledby="npTitle">
        <div class="sp-modal-head">
            ...
            <h3 class="sp-modal-title" id="npTitle">Record a Payment</h3>
            <p class="sp-modal-sub">Log a payment you've made to a seller. Link it to a purchase if applicable.</p>
        </div>
            ...
            <div class="sp-modal-body">
                ...
                <div class="sp-form-row">
                    ...
                    <div class="sp-form-field is-full">
                        <label for="payPurchase">Link to Purchase (optional)</label>
                        <div class="sp-form-input is-select">
                            <i class="bi bi-link-45deg"></i>
                            <select id="payPurchase" name="purchase_ref">
                                <option value="">— No linked purchase —</option>
                                <?php foreach ($openPurchases as $op): ?>
                                    <option value="<?= sanitize($op['reference_no']) ?>"
                                            data-balance="<?= (float) $op['balance'] ?>">
                                        <?= sanitize($op['reference_no']) ?> — <?= sanitize($op['seller_name']) ?> (bal: <?= peso($op['balance']) ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="sp-form-hint">Only purchases with an outstanding balance are listed.</div>
                    </div>
                    ...
                    <div class="sp-form-field">
                        ...
                        <div class="sp-form-input">
                            ...
                            <input type="number" id="payAmount" name="amount" min="0.01" step="0.01" placeholder="0.00" required>
                        </div>
                        <div class="sp-form-hint">In pesos (₱)</div>
                    </div>

                     <div class="sp-form-field">
                        <label for="payMethod">Method <span class="req">*</span></label>
                        <div class="sp-form-input is-select">
                            <i class="bi bi-credit-card-2-front"></i>
                            <select id="payMethod" name="method" required>
                    ...
                    <div class="sp-form-field">
                        ...
                        <div class="sp-form-input">
                            <i class="bi bi-calendar3"></i>
                            <input type="date" id="payDate" name="paid_at">
                        </div>
                    ...
                    <div class="sp-form-field">
                        ...
                        <div class="sp-form-input">
                            ...
                            <input type="text" id="payRef" name="reference_no" placeholder="Auto-generated if blank">
                        </div>
                    </div>
                    ...
                    <div class="sp-form-field is-full">
                        ...
                        <div class="sp-form-input">
                            ...
                            <textarea id="payNotes" name="notes" placeholder="e.g. Partial payment for last week's delivery…"></textarea>
                        </div>
                    </div>
                ...
                <div class="sp-form-summary">
                    ...
                    <span class="sp-form-summary-value" id="payTotal">₱0.00</span>
                </div>
...
<div class="sp-modal-backdrop" id="modalEdit">
    <div class="sp-modal" role="dialog" aria-modal="true" aria-labelledby="edTitle">
        <div class="sp-modal-head">
            ...
            <h3 class="sp-modal-title" id="edTitle">Update Payment</h3>
        ...
        <form id="editPaymentForm" autocomplete="off" style="display: contents;">
            ...
            <div class="sp-modal-body">
                ...
                <div class="sp-form-row">
                    ...
                    <div class="sp-form-field is-full">
                        <label for="edPurchase">Link to Purchase (optional)</label>
                        <div class="sp-form-input is-select">
                            <i class="bi bi-link-45deg"></i>
                            <select id="edPurchase" name="purchase_ref">
                                <option value="">— No linked purchase —</option>
                                <?php foreach ($openPurchases as $op): ?>
                                    <option value="<?= sanitize($op['reference_no']) ?>">
                                        <?= sanitize($op['reference_no']) ?> — <?= sanitize($op['seller_name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="sp-form-field">
                        ...
                        <div class="sp-form-input">
                            <i class="bi bi-cash-stack"></i>
                            <input type="number" id="edAmount" name="amount" min="0.01" step="0.01" required>
                        </div>
                    </div>
                    ...
                    <div class="sp-form-field">
                        ...
                        <div class="sp-form-input is-select">
                            <i class="bi bi-credit-card-2-front"></i>
                            <select id="edMethod" name="method" required>
                                <option value="cash">Cash</option>
                                <option value="bank">Bank Transfer</option>
                                <option value="gcash">GCash</option>
                                <option value="maya">Maya</option>
                    ...
                    <div class="sp-form-field">
                        ...
                        <div class="sp-form-input">
                            <i class="bi bi-calendar3"></i>
                            <input type="date" id="edDate" name="paid_at">
                        </div>
                    </div>
...
<div class="sp-modal-backdrop" id="modalDelete">
    <div class="sp-modal" role="dialog" aria-modal="true" aria-labelledby="dlTitle" style="max-width: 460px;">
        <div class="sp-modal-head">
            ...
            <p class="sp-modal-sub">This action cannot be undone. The payment record will be permanently removed.</p>
        </div>
        <div class="sp-modal-body">
...
(() => {
    ...
    document.querySelectorAll('[data-modal]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (name === 'newPayment') resetNewPaymentForm();
    ...
    /* ----------------------------------------------------------
       New Payment form
    ---------------------------------------------------------- */
    const payForm     = document.getElementById('newPaymentForm');
    const payPurchase = document.getElementById('payPurchase');
    ...
    function recalcNew() {
        ...
        payTotal.textContent = pesoFmt(amt);
    }
    payAmount?.addEventListener('input', recalcNew);

    // Auto-fill amount when linking to a purchase
    payPurchase?.addEventListener('change', () => {
        const opt = payPurchase.options[payPurchase.selectedIndex];
        const balance = parseFloat(opt?.dataset?.balance);
        if (!isNaN(balance) && balance > 0 && !payAmount.value) {
    ...
    payForm?.addEventListener('submit', async (e) => {
        ...
        const originalHtml = paySubmit.innerHTML;
        paySubmit.innerHTML = '<span class="spinner-border spinner-border-sm"></span> Saving…';

        try {
            const res = await fetch('<?= BASE_URL ?>buyer/save_payment.php', {
                method: 'POST',
            ...
            paySubmit.innerHTML = originalHtml;
        }
    });
    ...
    function openView(p) {
        ...
        vwBody.innerHTML = `
            <div class="sp-detail-hero">
                <div>
                    <div class="sp-detail-hero-label">Amount Paid</div>
            ...
            <div class="sp-detail-grid">
                ...
                <div class="sp-detail-item">
                    <div class="sp-detail-label">Payment Date</div>
                    <div class="sp-detail-value">${p.paid_at ? new Date(p.paid_at).toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' }) : '—'}</div>
                </div>
                <div class="sp-detail-item">
                    <div class="sp-detail-label">Method</div>
                    <div class="sp-detail-value">${mm.label}</div>
                </div>
                <div class="sp-detail-item">
                    <div class="sp-detail-label">Linked Purchase</div>
                    <div class="sp-detail-value is-mono">${esc(p.purchase_ref || '—')}</div>
                </div>
                ${p.notes ? `
    ...
    const edPurchase = document.getElementById('edPurchase');
    ...
    function recalcEdit() {
        edTotal.textContent = pesoFmt(parseFloat(edAmount?.value) || 0);
    }
    ...
    function openEdit(p) {
        ...
        edMethod.value   = (p.method || 'cash').toLowerCase();
        edDate.value     = p.paid_at ? p.paid_at.slice(0, 10) : '';
        edNotes.value    = p.notes || '';

        // If purchase_ref not in list, add it
        if (p.purchase_ref && ![...edPurchase.options].some(o => o.value === p.purchase_ref)) {
            const opt = document.createElement('option');
            opt.value = p.purchase_ref;
            opt.textContent = p.purchase_ref;
            edPurchase.appendChild(opt);
        }
        edPurchase.value = p.purchase_ref || '';
    ...
    edForm?.addEventListener('submit', async (e) => {
        ...
        const payload = {
            ...
            purchase_ref: edPurchase.value || '',
            amount:       parseFloat(edAmount.value) || 0,
            method:       edMethod.value || 'cash',
        ...
        try {
            const res = await fetch('<?= BASE_URL ?>buyer/update_payment.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload),
            });
    ...
    dlConfirmBtn?.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch('<?= BASE_URL ?>buyer/delete_payment.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
