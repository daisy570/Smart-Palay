...
if ($search !== '') {
    $where[] = '(reference_no LIKE ? OR buyer_name LIKE ? OR notes LIKE ?)';
    $like    = '%' . $search . '%';
...
$totalRows  = 0;
$totalPages = 1;
$statusCounts = ['all'=>0,'paid'=>0,'partial'=>0,'unpaid'=>0,'pending'=>0];
$summary = ['total_amount'=>0,'total_paid'=>0,'total_balance'=>0,'total_kg'=>0];
...
try {
    ...
    $countParams = [$userId];
    if ($search !== '') {
        $countWhere[] = '(reference_no LIKE ? OR buyer_name LIKE ? OR notes LIKE ?)';
...
try {
    ...
    if ($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $summary['total_balance'] = (float) $r['total_balance'];
...
try {
    ...
    // List
    $stmt = $pdo->prepare("
        SELECT id, reference_no, buyer_name, weight_kg, price_per_kg,
...
/* Buyers list for the New Sale modal */
$buyersList = [];
try {
    $stmt = $pdo->prepare("
        SELECT DISTINCT buyer_name FROM purchases
        WHERE seller_id = ? AND buyer_name <> ''
        ORDER BY buyer_name
    ");
    $stmt->execute([$userId]);
...
<title>My Sales | SmartPalay</title>
...
.sp-status-tabs{display:flex;flex-wrap:wrap;gap:6px}
.sp-status-tab{display:inline-flex;align-items:center;gap:7px;padding:8px 14px;border-radius:10px;background:var(--cream-2);border:1.5px solid var(--line);color:var(--text-2);font-size:.8rem;font-weight:600;text-decoration:none;transition:border-color .18s ease,background .18s ease,color .18s ease,transform .18s ease}
.sp-status-tab:hover{border-color:var(--line-3);color:var(--brown);transform:translateY(-1px)}
.sp-status-tab.is-active{background:linear-gradient(135deg,var(--gold-soft),#fff);border-color:rgba(176,100,30,.4);color:var(--gold);font-weight:700;box-shadow:0 4px 12px -6px rgba(176,100,30,.4)}
.sp-status-tab .count{padding:1px 8px;border-radius:999px;background:var(--paper-2);color:var(--text-2);font-size:.68rem;font-weight:700;font-family:'JetBrains Mono',monospace}
.sp-status-tab.is-active .count{background:rgba(176,100,30,.15);color:var(--gold)}
...
.sp-seller{display:flex;align-items:center;gap:10px}
.sp-seller-avatar{width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--gold),var(--gold-2));color:#fff;display:grid;place-items:center;font-family:'Fraunces',Georgia,serif;font-weight:700;font-size:.82rem;flex-shrink:0;box-shadow:0 4px 10px -4px rgba(176,100,30,.7)}
.sp-seller-name{font-weight:600;color:var(--brown);max-width:18ch;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
/* Detail view */
.sp-detail-hero{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:18px 20px;border-radius:14px;background:linear-gradient(135deg,var(--gold-soft),#fff);border:1px solid rgba(176,100,30,.22);flex-wrap:wrap}
...
.sp-detail-hero-value{font-family:'Fraunces',Georgia,serif;font-size:1.5rem;font-weight:700;color:var(--brown);letter-spacing:-.5px;line-height:1.1}
...
<aside class="sp-sidebar" id="sidebar">
    ...
    <nav class="sp-nav">
        ...
        <a href="<?= BASE_URL ?>seller/sales.php" class="sp-nav-link is-active" data-nav><i class="bi bi-bag-check"></i> My Sales</a>
...
<div class="sp-main">
    <header class="sp-topbar">
        <div class="sp-topbar-left">
            ...
            <div>
                <h1 class="sp-page-title">My Sales</h1>
    ...
    <main class="sp-content">
        ...
        <form class="sp-toolbar" method="get" action="">
            ...
            <div class="sp-status-tabs">
                <?php
                    $tabs = [
                        'all'     => ['All',     $statusCounts['all'],     'bi-grid'],
                        'paid'    => ['Paid',    $statusCounts['paid'],    'bi-check-circle'],
                        'partial' => ['Partial', $statusCounts['partial'], 'bi-circle-half'],
                        'unpaid'  => ['Unpaid',  $statusCounts['unpaid'],  'bi-x-circle'],
                        'pending' => ['Pending', $statusCounts['pending'], 'bi-hourglass-split'],
                    ];
            ...
            <div class="sp-filter-row">
                <div class="sp-field">
                    ...
                    <input type="search" name="q" value="<?= sanitize($search) ?>" placeholder="Search by reference, buyer, notes…">
                </div>
                ...
                <a href="<?= BASE_URL ?>seller/sales.php" class="sp-btn-ghost">
        ...
        <section class="sp-summary-strip">
            ...
            <div class="sp-summary-card">
                ...
                <div class="sp-summary-body">
                    <div class="sp-summary-label">Total Earned</div>
        ...
        <section class="sp-panel">
            <div class="sp-panel-head">
                ...
                <span class="sp-panel-meta">
                    Showing <strong><?= count($sales) ?></strong> of <strong><?= number_format($totalRows) ?></strong>
                </span>
            </div>
                    ...
                    <table class="sp-table">
                        ...
                        <tbody>
                            <?php foreach ($sales as $s): ?>
                                <?php
                                    [$badgeCls, $badgeIco, $badgeLabel] = statusMeta($s['status'] ?? 'pending');
                                    $initial = strtoupper(mb_substr(trim($s['buyer_name'] ?: '?'), 0, 1));
                                ?>
                                <tr data-sale='<?= htmlspecialchars(json_encode([
                                    'id'           => (int) $s['id'],
                                    'reference_no' => $s['reference_no'] ?? '',
                                    'buyer_name'   => $s['buyer_name'] ?? '',
                                    'weight_kg'    => (float) $s['weight_kg'],
                                    'price_per_kg' => (float) ($s['price_per_kg'] ?? 0),
                                    ...
                                    <td>
                                        <div class="sp-seller">
                                            ...
                                            <span class="sp-seller-name"><?= sanitize($s['buyer_name'] ?? '—') ?></span>
                                        </div>
                                    </td>
                                    <td><?= kg($s['weight_kg'] ?? 0) ?></td>
                                    <td><span class="sp-money"><?= peso($s['total_amount'] ?? 0) ?></span></td>
                                    <td>
                                        <span class="sp-money <?= ((float) $s['balance']) > 0 ? 'is-danger' : 'is-success' ?>">
                                            <?= peso($s['balance'] ?? 0) ?>
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td style="text-align:right;">
                                        <div class="sp-row-actions" onclick="event.stopPropagation();">
                                            <button type="button" class="sp-row-btn" title="View details" data-action="view" data-id="<?= (int) $s['id'] ?>">
                                                <i class="bi bi-eye"></i>
                                            </button>
                                            <button type="button" class="sp-row-btn" title="Edit" data-action="edit" data-id="<?= (int) $s['id'] ?>">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <button type="button" class="sp-row-btn is-danger" title="Delete" data-action="delete" data-id="<?= (int) $s['id'] ?>">
            ...
            <?php else: ?>
                <div class="sp-empty">
                    ...
                    <p>
                        ...
                        <?php else: ?>
                            Your palay sales records will appear here. Record your first sale to get started.
                        <?php endif; ?>
                    </p>
                    <div class="sp-empty-actions">
                        ...
                        <?php endif; ?>
                        <button type="button" class="sp-btn-primary" data-modal="newSale">
...
<div class="sp-modal-backdrop" id="modalView">
    <div class="sp-modal">
        <div class="sp-modal-head">
            <button type="button" class="sp-modal-close" data-close-modal><i class="bi bi-x-lg"></i></button>
            <span class="sp-modal-tag"><i class="bi bi-eye"></i> Sale Details</span>
            <h3 class="sp-modal-title" id="vwTitle">Sale Record</h3>
...
<div class="sp-modal-backdrop" id="modalEdit">
    <div class="sp-modal">
        <div class="sp-modal-head">
            <button type="button" class="sp-modal-close" data-close-modal><i class="bi bi-x-lg"></i></button>
            <span class="sp-modal-tag"><i class="bi bi-pencil"></i> Edit Sale</span>
            <h3 class="sp-modal-title">Update Sale</h3>
            <p class="sp-modal-sub">Adjust the details below and save your changes.</p>
        </div>

        <form id="editSaleForm" autocomplete="off" style="display:contents">
            <input type="hidden" id="edId">
            <div class="sp-modal-body">
                <div class="sp-form-row">
                    <div class="sp-form-field is-full">
                        <label for="edBuyer">Buyer <span class="req">*</span></label>
                        <div class="sp-form-input"><i class="bi bi-person"></i><input type="text" id="edBuyer" required></div>
                    </div>
                    <div class="sp-form-field">
                        <label for="edWeight">Weight <span class="req">*</span></label>
                        <div class="sp-form-input"><i class="bi bi-box-seam"></i><input type="number" id="edWeight" min="0.01" step="0.01" required></div>
                    </div>
                    <div class="sp-form-field">
                        <label for="edPrice">Price per kg <span class="req">*</span></label>
                        <div class="sp-form-input"><i class="bi bi-tag"></i><input type="number" id="edPrice" min="0.01" step="0.01" required></div>
                    </div>
                    <div class="sp-form-field">
                        <label for="edPaid">Amount Received</label>
                        <div class="sp-form-input"><i class="bi bi-cash-stack"></i><input type="number" id="edPaid" min="0" step="0.01"></div>
                    </div>
                    <div class="sp-form-field">
...
<div class="sp-modal-backdrop" id="modalDelete">
    <div class="sp-modal" style="max-width:460px">
        <div class="sp-modal-head">
            ...
            <h3 class="sp-modal-title" id="dlTitle">Delete this sale?</h3>
        ...
        <div class="sp-modal-foot">
            <button type="button" class="sp-btn-cancel" data-close-modal><i class="bi bi-x-lg"></i> Cancel</button>
            <button type="button" class="sp-btn-submit" id="dlConfirmBtn" style="background:linear-gradient(135deg,#A23A1A,#C4481F)">
...
(() => {
    ...
    /* Modal helpers */
    const modalNew    = document.getElementById('modalNewSale');
    const modalView   = document.getElementById('modalView');
    const modalEdit   = document.getElementById('modalEdit');
    const modalDelete = document.getElementById('modalDelete');
    const modals = { newSale: modalNew, view: modalView, edit: modalEdit, delete: modalDelete };
    ...
    document.querySelectorAll('[data-modal]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            if (btn.dataset.modal === 'newSale') resetNewSaleForm();
    ...
    function methodMeta(m) {
        ...
        const map = {
            ...
            bank:  { label: 'Bank Transfer', icon: 'bi-bank' },
            gcash: { label: 'GCash',         icon: 'bi-phone' },
            maya:  { label: 'Maya',          icon: 'bi-phone-fill' },
            check: { label: 'Check',         icon: 'bi-file-earmark-text' },
        };
        return map[m] || { label: m, icon: 'bi-three-dots' };
    }
    ...
    nsWeight?.addEventListener('input', recalcNew);
    nsPrice?.addEventListener('input', recalcNew);
    nsPaid?.addEventListener('input', recalcNew);
    ...
    document.querySelectorAll('[data-action]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const row = btn.closest('tr[data-sale]');
            if (!row) return;
            const data = JSON.parse(row.dataset.sale);
    ...
    function openView(s) {
        currentSale = s;
        const [badgeCls, badgeIco, badgeLabel] = statusLabel(s.status);
        vwTitle.textContent = s.reference_no || 'Sale Record';
        vwSub.textContent   = `Buyer: ${s.buyer_name || '—'} · ${s.created_at ? new Date(s.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : '—'}`;

        vwBody.innerHTML = `
            <div class="sp-detail-hero">
                <div>
                    <div class="sp-detail-hero-label">Total Amount</div>
                    <div class="sp-detail-hero-value">${pesoFmt(s.total_amount)}</div>
                </div>
            ...
            <div class="sp-detail-grid">
                <div class="sp-detail-item"><div class="sp-detail-label">Reference</div><div class="sp-detail-value is-mono">${esc(s.reference_no || '—')}</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Delivery Date</div><div class="sp-detail-value">${s.created_at ? new Date(s.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : '—'}</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Buyer</div><div class="sp-detail-value">${esc(s.buyer_name || '—')}</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Weight</div><div class="sp-detail-value">${Number(s.weight_kg).toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2})} kg</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Price per kg</div><div class="sp-detail-value">${pesoFmt(s.price_per_kg)}</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Amount Received</div><div class="sp-detail-value" style="color:var(--success)">${pesoFmt(s.amount_paid)}</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Balance</div><div class="sp-detail-value" style="color:${s.balance>0?'var(--danger)':'var(--success)'}">${pesoFmt(s.balance)}</div></div>
                <div class="sp-detail-item"><div class="sp-detail-label">Status</div><div class="sp-detail-value"><span class="sp-badge ${badgeCls}"><i class="bi ${badgeIco}"></i> ${badgeLabel}</span></div></div>
                ${s.notes ? `<div class="sp-detail-item is-full"><div class="sp-detail-label">Notes</div><div class="sp-detail-value is-muted" style="font-weight:500;font-size:.86rem;white-space:pre-wrap">${esc(s.notes)}</div></div>` : ''}
            </div>
        `;
        openModal(modalView);
    }
    ...
    vwEditBtn?.addEventListener('click', () => {
        ...
        if (currentSale) setTimeout(() => openEdit(currentSale), 220);
    });

    /* ---------- Edit modal ---------- */
    const edForm    = document.getElementById('editSaleForm');
    const edId      = document.getElementById('edId');
    const edBuyer   = document.getElementById('edBuyer');
    ...
    function openEdit(s) {
        currentSale = s;
        edId.value    = s.id;
        edBuyer.value = s.buyer_name || '';
        edWeight.value = s.weight_kg || '';
        edPrice.value  = s.price_per_kg || '';
        edPaid.value   = s.amount_paid || '';
        // Delivery Date — populate from the existing sale if present,
        // otherwise leave the field blank (no auto-filled "today").
        edDate.value   = s.created_at ? s.created_at.slice(0,10) : '';
    ...
    edForm?.addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch('<?= BASE_URL ?>seller/update_sale.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
            ...
            if (res.ok && data.success) {
                ...
                showToast(data.message || 'Could not update the sale.', 'error');
            }
        } catch (err) {
            showToast('Network error. Please try again.', 'error');
        } finally {
    ...
    function openDelete(s) {
        currentSale = s;
        dlRef.textContent    = s.reference_no || '—';
        dlAmount.textContent = pesoFmt(s.total_amount);
        openModal(modalDelete);
    }

    edDeleteBtn?.addEventListener('click', () => {
        closeModal(modalEdit);
        if (currentSale) setTimeout(() => openDelete(currentSale), 220);
    });
    ...
    dlConfirmBtn?.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch('<?= BASE_URL ?>seller/delete_sale.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: currentSale.id })
            });
            const data = await res.json().catch(() => ({}));
            if (res.ok && data.success) {
                ...
                showToast(data.message || 'Could not delete the sale.', 'error');
            }
        } catch (err) {
            showToast('Network error. Please try again.', 'error');
        } finally {
